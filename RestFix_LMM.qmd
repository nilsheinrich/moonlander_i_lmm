...
my_data = DataFrame(Arrow.Table("data/Experiment1_RestingFixations.arrow"))
...
```
we detect overparamterization. I suspect total_control_loss to hold no variance. Let's check it out
...
last(m_varyingSlope_complex_zc.λ)  # we only have one random effect: ID, but last() puts it into a nice matrix
# no zeroes on the diagonal
```
...
```
Not allowing correlation between random effects is favored by BIC (and AIC). Proceed with m_varyingSlope_complex_zc and starting to delete indivudal random effects.
...
```
_complex_zc is favored by BIC. Will proceed with that one.
...
```
_complex_zc is again favored by BIC.
...
```
Referring to BIC, _complex_zc is ahead.
...
```
We see a few data points which are aggressively adjusted, especially for input noise W and S. Here is where strength was borrowed from somewhere else and applied.
...
```
We see significance only for the interaction effect of N_visible_obstacles abd N_visible_drift_tiles. Input noise or the the individual continuous covariates don't seem to significanty affect the fixation duration in resting fixations.
...
Leaving **total_control_loss** out from the start:
```{julia}
#| label: m_varyingInt1
...
We detect significant effects for **N_visible_obstacles** , **N_visible_drift_tiles** individually but not for their interaction effect. Both levels of input noise: **input noise Weak** and **input noise Strong** also have an significant effect on fixation location in resting fixations.
