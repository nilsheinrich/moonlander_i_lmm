...
m_varyingSlope_complex = let
    varSlope = @formula(saccLandSite_dist_to_spaceship ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + (1 + N_visible_obstacles * N_visible_drift_tiles + input_noise | ID));
...
m_varyingSlope_zc = let
    varSlope = @formula(saccLandSite_dist_to_spaceship ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + zerocorr(1 + N_visible_obstacles + N_visible_drift_tiles + input_noise | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end
...
m_varyingSlope1 = let
    varSlope = @formula(saccLandSite_dist_to_spaceship ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + zerocorr(1 + N_visible_obstacles + N_visible_drift_tiles | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end
...
m_varyingSlope2 = let
    varSlope = @formula(saccLandSite_dist_to_spaceship ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + zerocorr(1 + N_visible_drift_tiles + input_noise | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end
...
m_varyingSlope3 = let
    varSlope = @formula(saccLandSite_dist_to_spaceship ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + zerocorr(1 + N_visible_obstacles + input_noise | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end
...
m_varyingInt1 = let
    varInt = @formula(saccLandSite_dist_to_closestObstacle ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + 
    (1 | ID) + 
...
```
We detect singularity... deleting single parameter

Starting with the interaction term within the random slopes:
```{julia}
#| label: m_varyingSlope_ni

m_varyingSlope_ni = let
    varInt = @formula(saccLandSite_dist_to_closestObstacle ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + 
    (1 + N_visible_obstacles + N_visible_drift_tiles + input_noise | ID));
    fit(MixedModel, varInt, my_data; contrasts=my_cake);
  end

issingular(m_varyingSlope_ni) # NOT overparameterized

```
That worked. This is our most complex model. Proceeding from here.
...
gof_summary = let
  nms = [:m_varyingSlope_ni, :m_varyingSlope_zc]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_ni, m_varyingSlope_zc)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
In this case, correlation between random slopes is favored. Proceeding with m_varyingSlope_ni.
...
gof_summary = let
  nms = [:m_varyingSlope_ni, :m_varyingSlope1]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_ni, m_varyingSlope1)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
gof_summary = let
  nms = [:m_varyingSlope_ni, :m_varyingSlope2]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_ni, m_varyingSlope2)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
gof_summary = let
  nms = [:m_varyingSlope_ni, :m_varyingSlope3]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_ni, m_varyingSlope3)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
Nope, referring to BIC (or AIC), the complex model without the interaction term wins out. m_varyingSlope_ni is therefore our selected model.
...
#| label: fig-cm_varyingSlope
#|
cm_varyingSlope = first(ranefinfo(m_varyingSlope_ni));
caterpillar!(Figure(; resolution=(800, 1200)), cm_varyingSlope; orderby=1)
```
...
#|
#| fig-cap: Shrinkage plots of the subject random effects in the chosen model
shrinkageplot!(Figure(; resolution=(1000, 1200)), m_varyingSlope_ni)
...
```{julia}
samples = parametricbootstrap(RNG, N_iterations, m_varyingSlope_ni)
tbl = samples.tbl
```
...
ridgeplot(samples; show_intercept=false, xlabel="Bootstrap density and 95%CI")
```
Of all fixed effects, only N_visible_obstacles has an effect on the distance between saccade landing sites and the closest obstacle, decreasing it significantly (-0.618816, -0.457202). Which is no suprise... more obstacles=less free space=eyes will always be closer to obstacles. We find no sognificant effects for either level of input noise (our hypothesis).
