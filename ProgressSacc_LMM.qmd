...
last(m_varyingSlope_complex.λ)  # no zeroes on the diagonal.

```
We will proceed with this complex model structure.
...
We see significance only for the **N_visible_obstacles** and the interaction effect **N_visible_obstacles*N_visible_drift_tiles** (confint), meaning these covariates significantly influence saccade amplitude in progressive saccades.
...
```
We will keep ID as random effect and include random slope effects.
