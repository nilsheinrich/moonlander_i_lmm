...
Results:
**N_visible_drift_tiles** significantly increases fixation duration (0.0604684,0.718685).
as does **input_noise strong-weak** (0.105359, 0.718845).
...
gof_summary = let
  nms = [:m_varyingSlope_complex_zc, :m_varyingSlope_complex_zc_ni]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex_zc, m_varyingSlope_complex_zc_ni)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
We find **no** significance for either level of input noise. We see a significant positive effect for **N_visible_drift_tiles** (0.00106098, 0.00520816).
...
issingular(m_varyingSlope_d) # Overparameterized

```

## Model selection
Going with m_varyingInt1 because increasing model complexity will only lead to overparameterization.
```{julia}
#| label: selected model
...
... as does **input noise: weak-none** (0.0498657, 0.141821).
However, **input noise: weak-none** significantly decreases distance to the closest obstacle in distant fixations (-0.177101, -0.090088).
