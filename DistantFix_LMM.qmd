...
```
The complex model without the interaction (m_varyingSlope_complex_ni) is favored when referring to BIC (not AIC) though.
...
m_varyingSlope_complex_zc = let
    formula = @formula(log(fixation_duration) ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise + 
...
**Discussing the results:** Only **N_visible_obstacles** affects the fixation duration in distant fixations (0.00303191, 0.0179888).
...
```
The model with interaction wins out slightly when referring to BIC (even more with AIC though). Proceeding with m_varyingSlope_complex.
...
We find **no** significance for either level of input noise. We see a significant effect for **N_visible_obstacles** (0.00085836, 0.00168293) and for the interaction term of **N_visible_obstacles*N_visible_drift_tiles** (0.000146243, 0.00130684).
...
**N_visible_drift_tiles** significantly increases the distance to the closest obstacle in distant fixations (0.0228692, 0.0763562). Either level of input noise has no effect on the predicted variable.
