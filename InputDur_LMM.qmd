...
gof_summary = let
  nms = [:m_varyingSlope_complex_nc, :m_varyingSlope_complex_ni]
  mods = eval.(nms)
...
```
BIC favors _complex_nc and therefore correlation between random effects (AIC favors correlation though). Proceeding with m_varyingSlope_complex_nc and starting to delete individual random effects.
...
gof_summary = let
  nms = [:m_varyingSlope_complex_nc, :m_varyingSlope1]
  mods = eval.(nms)
...
gof_summary = let
  nms = [:m_varyingSlope_complex_nc, :m_varyingSlope2]
  mods = eval.(nms)
...
gof_summary = let
  nms = [:m_varyingSlope_complex_nc, :m_varyingSlope3]
  mods = eval.(nms)
...
```
We detect significant effects for **N_visible_obstcales**, **N_visible_drift_tiles**, and their **interaction** effect. The CIs show no evidence for either level of input_noise significantly affectling input duration.
