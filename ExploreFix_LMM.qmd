...
```
We only see a significant effect (increase) for **N_visible_obstacles** on fixation duration in exploring fixations.
...
confint(samples)
```
**N_visible_obstacles**, **N_visible_drift_tiles**, and their **interaction** term is significant.
...
We find **no** significance for either level of input noise. We however find that **N_visible_obstacles** and **N_visible_drift_tiles** are significantly **decreasing** distance to the spaceship within exploring fixations and that the **interaction** of both significantly **increases** the distance to spaceship in exploring fixations. The interaction effect tells us that the effect of N_visible_obstacles depends on the level of N_visible_drift_tiles and vice versa. 

# Sanity check - predicting distance to spaceship
```{julia}
...
StatsPlots.histogram(my_data.distance_to_spaceship, norm=false, title="Density of saccade amplitude")
#density(my_data.distance_to_spaceship, title="PDF of saccade amplitude")

```
We will log transform the predicted variable distance_to_spaceship.

## Building various models

### Only varying intercept model
```{julia}
#| label: m_varyingInt1

m_varyingInt1 = let
    formula = @formula(log(distance_to_spaceship) ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise 
    + (1 | ID)
...
VarCorr(m_varyingInt1)
last(m_varyingInt1.lambda)

```

Leaving out **total_control_loss** and only including **ID** as random intercept:
```{julia}
#| label: m_varyingInt2
...
```
Proceeding from here. Next up, exploring random slope effects.
...
```
This is the most complex model that we can build. We will start to delete individual random effects.
...
```{julia}
#| label: m_varyingSlope_nizc

m_varyingSlope_nizc = let
    formula = @formula(log(distance_to_spaceship) ~ 1 + N_visible_obstacles * N_visible_drift_tiles + input_noise 
    + zerocorr(1 + N_visible_obstacles + N_visible_drift_tiles + input_noise | ID));
    fit(MixedModel, formula, my_data; contrasts=my_cake);
  end

issingular(m_varyingSlope_nizc) # NOT overparameterized
...
gof_summary = let
  nms = [:m_varyingSlope_complex, :m_varyingSlope_ni, :m_varyingSlope_zc, :m_varyingSlope_nizc]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex, m_varyingSlope_ni, m_varyingSlope_zc, m_varyingSlope_nizc)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```
m_varyingSlope_zc, the model with no correlation between the random effects but with interaction wins out. Proceeding from here.
...
```
Referring to BIC (or AIC), input_noise should be kept as random slope.
...
```
N_visible_obstacles should be kept as well.
...
We can verify having stated zero correlation between random effects by referring to a caterpillar plot.
```{julia}
#| fig-cap1: Prediction intervals on subject random effects for model m_varyingSlope
#| label: fig-cm_varyingSlope
#|
...
confint(samples)
```
Only significant main effect is N_visible_obstacles: significantly decreasing distance to closest obstace (-0.0180576, -0.00743874). - not surprising...
