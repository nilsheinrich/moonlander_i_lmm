...
Deleting the interaction effect within the random effects structure.
```{julia}
...
```{julia}
#MixedModels.likelihoodratiotest(m_varyingSlope_complex, m_varyingSlope_complex_zc)
...
```
No correlation between random effects is favored (BIC and AIC agree here). Will proceed with m_varyingSlope_complex_zc.
...
last(m_varyingSlope1.λ)  # we only have one random effect: ID, but last() puts it into a nice matrix
# no zeroes on the diagonal
```
...
last(m_varyingSlope2.λ)  # we only have one random effect: ID, but last() puts it into a nice matrix
# no zeroes on the diagonal
```
...
```
m_varyingSlope_complex_zc is still favored by BIC (or AIC).
...
## Shrinkage plot
This plot shows where strength was borrowed and applied to the data.
```{julia}
#| code-fold: true
#| label: fig-shrinkage
#|
...
```{julia}
samples = parametricbootstrap(N_iterations, 100, m_varyingSlope_complex_zc)
tbl = samples.tbl
```

For a parametric bootstrap, you actually want to go for 5000 or 10000 replicates.
...
and at the standard deviations
```{julia}
#plt = data(tbl) * mapping(
#  [:σ1, :σ2, :σ3] .=> "Bootstrap replicates of standard deviations";
#  color=dims(1) => renamer(["N_obstacles", "N_drift", "N_obstacles * N_drift"])
#  ) * AoG.density()
#draw(plt; figure=(;supertitle="Parametric bootstrap σ estimates of variance components"))
```
...
again native plot for betas but with intercept:
```{julia}
#draw(
#  data(samples.β) * mapping(:β; color=:coefname) * AoG.density();
...
```
We only see a significant effect for **N_visible_obstacles** on fixation duration in exploring fixations. Cannot visually determine significance for the interaction effect, but confint says **NO**
...
```
We detect singularity. We will adjust the random effects ditching the interaction effect.
...
```
Zero correlation is favored by BIC (not by AIC though). Proceeding with m_varyingSlope_complex_zc.
...
```
The one with input noise within the random effects structure (_complex_zc) is favored.
...
```{julia}
#MixedModels.likelihoodratiotest(m_varyingSlope3, m_varyingSlope_complex)
...
MixedModels.PCA(m_varyingSlope_complex_zc)
```
no valuable information due to zerocorr
...
```{julia}
#plt = data(tbl) * mapping(
#  [:σ1, :σ2, :σ3, :σ4, :σ5, :σ6] .=> "Bootstrap replicates of standard deviations";
#  color=dims(1) => renamer(["N_obstacles", "N_drift", "N_obstacles * N_drift"])
#  ) * AoG.density()
#draw(plt; figure=(;supertitle="Parametric bootstrap σ estimates of variance components"))
```
...
draw(plt; figure=(;supertitle="Parametric bootstrap β estimates of variance components"))
```
We also find here no intersection of the 95% CI and x=0 line.

We find **no** significance for either level of input noise. We however find that N_visible_obstacles, N_visible_drift_tiles, and the interaction of both affects the fixation location (distance to spaceship) in exploring fixations significantly.
