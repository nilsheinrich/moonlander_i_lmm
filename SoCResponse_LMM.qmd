...
```{julia}
#| label: m_varyingSlope_complex_zc_done

m_varyingSlope_complex_zc_done = let
    varSlope = @formula(SoC ~ 1 + done + level_difficulty * drift + input_noise + N_fixations + N_saccades + N_consecutive_crash_success + zerocorr(1 + done | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end

issingular(m_varyingSlope_complex_zc_done) # NOT overparamterized
```

```{julia}

gof_summary = let
  nms = [:m_varyingSlope_complex_zc, :m_varyingSlope_complex_zc_done]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex_zc, m_varyingSlope_complex_zc_done)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
...
```{julia}
#| label: m_varyingSlope_complex_zc_drift

m_varyingSlope_complex_zc_drift = let
    varSlope = @formula(SoC ~ 1 + done + level_difficulty * drift + input_noise + N_fixations + N_saccades + N_consecutive_crash_success + zerocorr(1 + drift | ID));
    fit(MixedModel, varSlope, my_data; contrasts=my_cake);
  end

issingular(m_varyingSlope_complex_zc_drift) # NOT overparamterized
```

```{julia}

gof_summary = let
  nms = [:m_varyingSlope_complex_zc, :m_varyingSlope_complex_zc_drift]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex_zc, m_varyingSlope_complex_zc_drift)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
```{julia}
#| label: m_varyingSlope_complex_zc_input_noise

m_varyingSlope_complex_zc_input_noise = let
...
issingular(m_varyingSlope_complex_zc_input_noise) # singularity
```

```{julia}
#| label: m_varyingSlope_complex_zc_ld

m_varyingSlope_complex_zc_ld = let
...
issingular(m_varyingSlope_complex_zc_ld) # singularity
```

```{julia}
#| label: m_varyingSlope_complex_zc_Nfix

m_varyingSlope_complex_zc_Nfix = let
...
issingular(m_varyingSlope_complex_zc_Nfix) # NO singularity
```

```{julia}
#| label: m_varyingSlope_complex_zc_Nsacc

m_varyingSlope_complex_zc_Nsacc = let
...
issingular(m_varyingSlope_complex_zc_Nsacc) # singularity
```

```{julia}
#| label: m_varyingSlope_complex_zc_ccs

m_varyingSlope_complex_zc_ccs = let
...
issingular(m_varyingSlope_complex_zc_ccs) # NO singularity
```
...
gof_summary = let
  nms = [:m_varyingSlope_complex_zc, :m_varyingSlope_complex_zc_ccs]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex_zc, m_varyingSlope_complex_zc_ccs)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
...
gof_summary = let
  nms = [:m_varyingSlope_complex_zc, :m_varyingSlope_complex_zc_Nfix]
  mods = eval.(nms)
  lrt = MixedModels.likelihoodratiotest(m_varyingSlope_complex_zc, m_varyingSlope_complex_zc_Nfix)
  DataFrame(;
    name = nms, 
    dof=dof.(mods),
    deviance=round.(deviance.(mods), digits=0),
