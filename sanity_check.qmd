...
m_varyingInt1 = let
    varInt = @formula(done ~ 1 + level_difficulty * drift + input_noise + (1 | ID));
    fit(MixedModel, varInt, my_data, dist; contrasts=my_cake);
  end
...
m_varyingSlope_complex = let
    varInt = @formula(done ~ 1 + level_difficulty * drift + input_noise + 
...
levels = Dict(
  :level_difficulty => ["easy", "medium", "hard"], :drift => ["false", "true"], :input_noise => ["N", "W", "S"]
)

preds = effects(levels, m_varyingInt1; invlink=AutoInvLink())
```
