...
my_data = DataFrame(Arrow.Table("data/Experiment1_AllFixations.arrow"))
...
m_varyingInt1 = let
    formula = @formula(distant_fixation ~ 1 + N_visible_obstacles + N_visible_drift_tiles + input_noise 
    + (1 | ID));
...
```
Input noise is also crucial as random slope. We will select the complex model as our final model.
...
## Results
We so **no** significant effects. The probability to initate a fixation that is distant (vs. close) to the spaceship is not influenced by any of our covariates.

N_visible_obstacles *nearly* reaches significance with p=0.0537, potentially increasing the probability to initate a distant fixation.
